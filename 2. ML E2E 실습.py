...
# MAGIC - 등록된 모델을 Spark UDF를 통해 다른 데이터셋에 적용
...
import os
os.environ["DATABRICKS_TOKEN"] = "dapi7e2b0f5824ab66db60c6ac64982add05"
...
# 새로운 데이터 뭉치를 시뮬레이션 해보기 위해 이미 존재하고 있는 X_train 데이터를 Delta 테이블 형식으로 저장합니다. 
# 실제 세상에서는 이 부분은 새로운 배치성 데이터가 될 것입니다.
spark_df = spark.createDataFrame(data)
table_path = "dbfs:/hol/delta/wine_data2"
# 이 셀이 이전에도 수행되었을 수도 있었기 때문에 table_path 아래 컨텐츠를 우선 삭제 합니다.
#dbutils.fs.rm(table_path, True)
spark_df.write.format("delta").save(table_path)
spark_df.write.mode("overwrite").saveAsTable("wine_quality2")
...
# MAGIC %md ##배치 추론
# MAGIC 
# MAGIC 많은 시나리오에서 여러분은 새로운 데이터에 대한 평가를 하고 싶으실 수 있습니다. 예를 들어 새롭게 수집된 배치성 데이터나 동일한 데이터로 두 가지 모델의 성능을 비교해야 할 수도 있습니다.
# MAGIC 
# MAGIC 아래 코드는 Delta 테이블로 저장되어 있는 데이터를 모델로 평가하고 Spark로 동시에 연산할 수 있도록 합니다.
...
dbutils.fs.rm(table_path, True)
spark_df.write.format("delta").save(table_path)
spark_df.write.mode("overwrite").saveAsTable("wine_quality")

# COMMAND ----------

# MAGIC %md 새로운 모델을 Spark UDF로 로딩하여 Delta 테이블에 적용될 수 있도록 합니다.
...
# 각 행은 이제 에측치를 포함하고 있습니다. XGBoost 함수는 기본적으로 확률을 output으로 제공하지 않습니다. 따라서 예측은 0과 1 사이의 값으로 표현됩니다.
display(new_data)
